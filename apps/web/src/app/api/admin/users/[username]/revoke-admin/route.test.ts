...
//
// Mirrors `../grant-admin/route.test.ts`.
...
describe("POST /api/admin/users/[username]/revoke-admin", () => {
  ...
  it("forwards the X-API-Key and the caller's Bearer token, returning the result on 200", async () => {
    const roleGrant = { username: "alice", is_admin: false };
  ...
  it("collapses any other backend status to a generic 502 failure", async () => {
    ...
    expect(body).toEqual({ error: "revoke_admin_failed" });
  });
});
