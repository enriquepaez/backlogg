...
/**
 * Public followers list for one user (FE-23), `/u/{username}/followers`.
 * Sibling of `/u/{username}/library` (FE-20) — same shape: validate the
 * username via `getUserProfile` (only a backend-confirmed 404 goes through
 * `notFound()`, same split as `getItemDetail`'s doc comment), then load the
 * paginated list itself.
 */
export default async function UserFollowersPage({
  params,
  searchParams,
}: PageProps<"/[locale]/u/[username]/followers">) {
  ...
  const [t, profileResult, result] = await Promise.all([
    ...
    getFollowers(username, { page, limit: FOLLOW_PAGE_SIZE }),
  ]);
    ...
    <div className="mx-auto flex w-full max-w-2xl flex-col gap-8 px-6 py-16">
      <div className="flex flex-col gap-2">
        ...
        <h1 className="text-3xl font-semibold tracking-tight">
          {t("followersHeading", { name: profile.display_name ?? profile.username })}
        </h1>
      </div>
      ...
      {!result.ok ? (
        ...
        <>
          ...
          <FollowPagination href={`/u/${username}/followers`} page={result.page} totalPages={totalPages} />
        </>
      )}
    </div>
  );
