...
/**
 * `/admin/games` (FE-33): stub, reachable from `<AdminSidebar>` but with no
 * functionality yet — FE-34 (`admin_catalog_backoffice`) fills this in. Same
 * shape as its `movies`/`series`/`books` siblings — see
 * `@/app/[locale]/admin/movies/page.tsx`'s own doc comment.
 */
export default async function AdminGamesStubPage({
  params,
  ...
  return <AdminComingSoon section={t("games")} />;
}
