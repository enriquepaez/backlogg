...
/**
 * `/admin/books` (FE-34): fills in the stub FE-33 left in place. Same shape
 * as its `movies`/`series`/`games` siblings, including the second-pass
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex flex-col gap-2">
        <h1 className="text-2xl font-semibold tracking-tight">{tSidebar("books")}</h1>
        <p className="text-sm text-muted-foreground">{t("description", { type: tSidebar("books") })}</p>
      </div>
      ...
      {!result.ok ? (
        ...
        <>
          ...
          {result.items.length === 0 ? (
            ...
            <>
              <AdminCatalogTable type="book" items={result.items} dateLabel={t("dateLabel.book")} />
