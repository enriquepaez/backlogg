...
const { default: AdminSeriesPage } = await import("./page");
...
describe("AdminSeriesPage", () => {
  it("renders the heading and description", async () => {
    listCatalog.mockResolvedValue({ ok: true, items: [seriesItem], total: 1, page: 1, limit: 24 });
    getGenres.mockResolvedValue([]);

    render(await AdminSeriesPage({
      params: Promise.resolve({ locale: "en" }),
      searchParams: Promise.resolve({}),
    }));

    expect(screen.getByRole("heading", { name: "series" })).toBeInTheDocument();
    expect(screen.getByText('description:{"type":"series"}')).toBeInTheDocument();
  });

  it("parses genre/sort/page from searchParams and forwards them to listCatalog/getGenres", async () => {
    listCatalog.mockResolvedValue({ ok: true, items: [seriesItem], total: 1, page: 2, limit: 24 });
    getGenres.mockResolvedValue([]);

    await AdminSeriesPage({
      params: Promise.resolve({ locale: "en" }),
      searchParams: Promise.resolve({ genre: "action", sort: "title_asc", page: "2" }),
    });

    expect(listCatalog).toHaveBeenCalledWith("series", { genre: "action", sort: "title_asc", page: 2 });
    expect(getGenres).toHaveBeenCalledWith("series");
  });
  ...
  it("falls back to defaults for missing/invalid searchParams", async () => {
    ...
    expect(listCatalog).toHaveBeenCalledWith("series", {
      genre: undefined,
      sort: "rating_desc",
      page: 1,
      search: undefined,
  ...
  it("forwards the parsed filters to AdminCatalogFilters and AdminCatalogPagination", async () => {
    listCatalog.mockResolvedValue({ ok: true, items: [seriesItem], total: 41, page: 1, limit: 24 });
    getGenres.mockResolvedValue([]);

    render(
      await AdminSeriesPage({
        params: Promise.resolve({ locale: "en" }),
        searchParams: Promise.resolve({ search: "breaking", date_from: "2008-01-01", rating_external_min: "8" }),
      }),
    );
    ...
    expect(filtersProps).toMatchObject({
      selectedSearch: "breaking",
      selectedDateFrom: "2008-01-01",
  ...
  it("renders the results count, table and pagination with the right props when items are present", async () => {
    ...
    expect(tableProps.type).toBe("series");
    expect(tableProps.items).toEqual([seriesItem]);
    expect(tableProps.dateLabel).toBe("dateLabel.series");

    const pagination = screen.getByTestId("admin-catalog-pagination");
    const paginationProps = JSON.parse(pagination.getAttribute("data-props")!);
    expect(paginationProps).toMatchObject({ type: "series", page: 1, totalPages: 2 });

    const filters = screen.getByTestId("admin-catalog-filters");
    const filtersProps = JSON.parse(filters.getAttribute("data-props")!);
    expect(filtersProps).toMatchObject({ type: "series", selectedSort: "rating_desc" });
