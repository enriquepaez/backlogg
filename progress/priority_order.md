...
| **#23** duplicado huérfano por renombrado en la fuente | medium | **YA DIMENSIONADO** (2026-09-07): 149 targets perdidos de 68.046 — movies 142 (0,25 %), series 7 (0,064 %). Era la condición que esta cola esperaba para decidirlo. Mecanismo confirmado: el slug hace de identidad y `uq_item_source` bloquea el segundo enlace. Los 149 siguen en `seed_targets`, reabribles. **Toca decidir**: entra ahora o se acepta el 0,22 % |
...
| ~~**#15** credits vacíos en el catálogo~~ ✅ **resolved 2026-09-07** | high | **Cerrado con la siembra, medido y no supuesto**: grafo de credits al 99,8 % en movies, 82,6 % en series y ~100 % en books, frente al 21/11/28 % que abrió el issue. ~~Sus dos mitades están resueltas en el papel, pero sigue `open` a propósito~~ (ver más abajo): movies/series/books se disuelve con el borrado —que aún no se ha ejecutado— y games se **podó** el 2026-09-04 |
...
| 6 | ~~**89** `credits_people_storage_redesign`~~ ✅ **done 2026-09-07** | Liberó **152 MB** (grafo de personas 266 → 114 MB) y dejó el cluster en 385 MB. Medición y decisión en `progress/measure_89.md`; despliegue en `progress/deploy_89.md` |
| 7 | ~~**Siembra de producción**~~ ✅ **HECHA el 2026-09-07**: 97.056 ítems (movies 57.024 · series 10.873 · books 19.159 · games 10.000 topados). Cerró la feature 74 y el issue #15. Desbloqueada el mismo día: el issue #28 se cerró con la feature 91, que retiró `catalog_search` y con ella el refresh. Cluster en 343 MB con 169 de holgura. ~~BLOQUEADA por el issue #28~~ (descubierto el 2026-09-07 en la QA de despliegue de la 89): `REFRESH MATERIALIZED VIEW CONCURRENTLY` duplica `catalog_search` —137 MB sobre 127 libres—, así que hoy no cabe, y a catálogo completo serían ~191 MB por duplicado. Resolver antes. Después, retomar donde quedó: los 795 movies pendientes siguen en `seed_targets`, la fase `load` de books es idempotente, y series entera. Nada de las 2h44m ya invertidas se repite |
