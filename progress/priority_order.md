...
| 7 | **Siembra de producción** | ⛔ **BLOQUEADA por el issue #28** (descubierto el 2026-09-07 en la QA de despliegue de la 89): `REFRESH MATERIALIZED VIEW CONCURRENTLY` duplica `catalog_search` —137 MB sobre 127 libres—, así que hoy no cabe, y a catálogo completo serían ~191 MB por duplicado. Resolver antes. Después, retomar donde quedó: los 795 movies pendientes siguen en `seed_targets`, la fase `load` de books es idempotente, y series entera. Nada de las 2h44m ya invertidas se repite |
