...
range filters. No ``rating_internal_*`` filter (out of scope for feature 69)
and no extra ``search``/substring param (``q`` already covers that).

Since feature 91 these filters run against the four content tables directly
(a ``UNION ALL`` over their ``search_vector`` generated columns) rather than
against a materialized view — the date bounds therefore hit each type's own
date column, which ``tests/test_search_expression_index.py`` pins per type.
"""
...
async def test_search_without_q_ingest_mocks_never_called(client, db):
