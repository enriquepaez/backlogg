...
async def test_hide_review_without_key_returns_401(client):
    response = await client.post("/admin/reviews/1/hide")
    assert response.status_code == 401


async def test_hide_unknown_review_returns_404(client):
    response = await client.post("/admin/reviews/999999999/hide", headers=_admin())
    assert response.status_code == 404


async def test_unhide_unknown_review_returns_404(client):
    response = await client.post("/admin/reviews/999999999/unhide", headers=_admin())
    assert response.status_code == 404
...
async def test_hide_review_excludes_from_item_ratings_and_aggregate(client, db):
    ...
    before_list = await client.get("/movies/mod-hide-movie-1/ratings")
    assert before_list.json()["total"] == 1
    before_detail = await client.get("/movies/mod-hide-movie-1")
    assert before_detail.json()["rating_internal"] == 4
    assert before_detail.json()["rating_count_internal"] == 1

    hide = await client.post(f"/admin/reviews/{rating_id}/hide", headers=_admin())
    assert hide.status_code == 200
    assert hide.json() == {"id": rating_id, "is_hidden": True}

    # After hiding: gone from the list and no longer counted in the aggregate.
    after_list = await client.get("/movies/mod-hide-movie-1/ratings")
    assert after_list.json()["total"] == 0
    after_detail = await client.get("/movies/mod-hide-movie-1")
...
async def test_hide_review_excludes_from_user_reviews(client, db):
    ...
    before = await client.get("/users/mod-hide-author-2/reviews")
    assert before.json()["total"] == 1

    await client.post(f"/admin/reviews/{rating_id}/hide", headers=_admin())

    after = await client.get("/users/mod-hide-author-2/reviews")
...
async def test_hide_review_excludes_from_feed(client, db):
    ...
    before = await client.get("/feed?tab=popular", headers=_auth(viewer_token))
    assert any(item["id"] == rating_id for item in before.json()["items"])

    await client.post(f"/admin/reviews/{rating_id}/hide", headers=_admin())

    after = await client.get("/feed?tab=popular", headers=_auth(viewer_token))
...
async def test_unhide_restores_review_and_aggregate(client, db):
    ...
    await client.post(f"/admin/reviews/{rating_id}/hide", headers=_admin())
    unhide = await client.post(f"/admin/reviews/{rating_id}/unhide", headers=_admin())
    assert unhide.status_code == 200
    assert unhide.json() == {"id": rating_id, "is_hidden": False}

    listing = await client.get("/movies/mod-hide-movie-4/ratings")
    assert listing.json()["total"] == 1
    detail = await client.get("/movies/mod-hide-movie-4")
...
async def test_hide_is_idempotent(client, db):
    ...
    first = await client.post(f"/admin/reviews/{rating_id}/hide", headers=_admin())
    second = await client.post(f"/admin/reviews/{rating_id}/hide", headers=_admin())
...
async def test_ban_user_without_key_returns_401(client):
    response = await client.post("/admin/users/whoever/ban")
    assert response.status_code == 401


async def test_ban_unknown_user_returns_404(client):
    response = await client.post("/admin/users/nobody-mod-ban/ban", headers=_admin())
    assert response.status_code == 404


async def test_unban_unknown_user_returns_404(client):
    response = await client.post("/admin/users/nobody-mod-ban/unban", headers=_admin())
    assert response.status_code == 404
...
async def test_banned_user_cannot_login(client, db):
    ...
    ban = await client.post("/admin/users/mod-ban-login-user/ban", headers=_admin())
    assert ban.status_code == 200
    assert ban.json() == {"username": "mod-ban-login-user", "is_banned": True}

    login = await client.post(
        "/auth/login", json={"username": "mod-ban-login-user", "password": "s3cret-pw"}
    )
...
async def test_banned_user_cannot_refresh(client, db):
    ...
    login = await client.post(
        "/auth/login", json={"username": "mod-ban-refresh-user", "password": "s3cret-pw"}
    )
    refresh_token = login.json()["refresh_token"]

    await client.post("/admin/users/mod-ban-refresh-user/ban", headers=_admin())
...
async def test_unban_restores_login(client, db):
    ...
    await client.post("/admin/users/mod-unban-login-user/ban", headers=_admin())

    unban = await client.post("/admin/users/mod-unban-login-user/unban", headers=_admin())
    assert unban.status_code == 200
    assert unban.json()["is_banned"] is False

    login = await client.post(
        "/auth/login", json={"username": "mod-unban-login-user", "password": "s3cret-pw"}
    )
...
async def test_ban_hides_user_reviews_from_all_surfaces(client, db):
    ...
    await client.post("/admin/users/mod-ban-author-1/ban", headers=_admin())

    ratings = await client.get("/movies/mod-ban-movie-1/ratings")
    assert ratings.json()["total"] == 0

    reviews = await client.get("/users/mod-ban-author-1/reviews")
    assert reviews.status_code == 200
    assert reviews.json()["total"] == 0

    detail = await client.get("/movies/mod-ban-movie-1")
    ...
    feed = await client.get("/feed?tab=popular", headers=_auth(viewer_token))
...
async def test_unban_restores_user_reviews_and_aggregate(client, db):
    ...
    await client.post("/admin/users/mod-ban-author-2/ban", headers=_admin())
    await client.post("/admin/users/mod-ban-author-2/unban", headers=_admin())

    ratings = await client.get("/movies/mod-ban-movie-2/ratings")
    assert ratings.json()["total"] == 1
    detail = await client.get("/movies/mod-ban-movie-2")
...
async def test_ban_is_idempotent(client, db):
    ...
    first = await client.post("/admin/users/mod-ban-idem-user/ban", headers=_admin())
    second = await client.post("/admin/users/mod-ban-idem-user/ban", headers=_admin())
    assert first.status_code == 200
    assert second.status_code == 200
