...
async def _snapshot(db, slug: str) -> dict:
    ...
    # Feature 91: ``search_vector`` is generated by Postgres from title and
    # overview, so the two write routes must land on the identical vector —
    # which is worth asserting, since a divergence would only ever show up as
    # a search result missing for rows written by one route and not the other.
    # Selected explicitly rather than read off the ORM object: the mapped
    # column is deferred (see backlogg/movies/models.py) and touching the
    # attribute would emit a lazy load, which asyncio cannot service.
    search_vector = (
        await db.execute(select(Movie.search_vector).where(Movie.id == movie.id))
    ).scalar_one()
    snapshot = {
