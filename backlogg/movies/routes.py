...
@router.get(
    ...
    summary="Movie adaptations",
    description=(
        "Adaptations and derived works of this movie, as declared by Wikidata. Each entry carries "
        "the related item's item_type/slug and the direction; the related item may be of any "
        "type, including another movie. Empty list when there are none; 404 only for an "
        "unknown slug."
    ),
)
async def get_movie_adaptations(slug: str, db: AsyncSession = Depends(get_db)):
    return await recommendations_service.get_item_adaptations(db, item_type="MOVIE", slug=slug)
