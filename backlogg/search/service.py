...
(only as many as will actually be shown to the caller) and re-executes the
local search query before returning. Since feature 91 there is no refresh
step in between: ``search_vector`` is a generated column on the four content
tables, so an ingested row is searchable as soon as its own transaction
commits.
...
class SearchService:
    ...
    async def search(
        ...
        * It is page 1 and this exact ``q``/``item_type`` pair has not been
        ...
        # No refresh step: the rows the fan-out just committed are already
        # searchable (their search_vector was generated by the same INSERT).
